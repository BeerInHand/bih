module Beerinhand::Constants
  ...
  def valid_volume_units
    @valid_volume_units ||= Beerinhand::Units::Volume.valid_units.map(&:to_s).map(&:titleize)
  end

  def valid_weight_units
    @valid_weight_units ||= Beerinhand::Units::Weight.valid_units.map(&:to_s).map(&:titleize)
  end
